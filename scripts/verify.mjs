...
//
// bash 実装から Node 実装へ移行(HC-001): Windows の npm scripts は cmd 経由で、
// bash が WSL に解決されたり env プレフィックス構文が使えなかったりするため、
// シェル非依存の Node で書く。cmd / PowerShell / Git Bash / Linux CI で同一動作。
//
// 使い方:
